...
test.describe('Anti-King Variant', () => {
    ...
    test('anti-king variant option is visible in mode select', async ({ page }) => {
        await page.click('.btn-new-game');
        await page.waitForSelector('#screen-mode-select.active');
        await expect(page.locator('.variant-option[data-variant="anti-king"]')).toBeVisible();
    });

    test('anti-king variant can be selected', async ({ page }) => {
        await page.click('.btn-new-game');
        await page.waitForSelector('#screen-mode-select.active');
        await page.click('.variant-option[data-variant="anti-king"]');
        const btn = page.locator('.variant-option[data-variant="anti-king"]');
    ...
    test('board variant is set to anti-king', async ({ page }) => {
        ...
        expect(variant).toBe('anti-king');
    });

    test('anti-king puzzle has no king-diagonal duplicates', async ({ page }) => {
        await startNewGame(page, 'easy', { variant: 'anti-king' });
    ...
    test('anti-king game shows variant info in info bar', async ({ page }) => {
        await startNewGame(page, 'easy', { variant: 'anti-king' });
        await expect(page.locator('.info-variant')).toBeVisible();
        await expect(page.locator('.variant-value')).toHaveText('안티킹');
    });

    // --- Highlight ---

    test('selecting cell highlights king-diagonal cells', async ({ page }) => {
        await startNewGame(page, 'easy', { variant: 'anti-king' });

        // Click cell (4,4) - center of 9x9
        await page.click('.cell[data-row="4"][data-col="4"]');

        // King diagonal cells from (4,4): (3,3),(3,5),(5,3),(5,5)
    ...
    test('king-diagonal conflict is detected', async ({ page }) => {
        await startNewGame(page, 'easy', { variant: 'anti-king' });
        ...
        const hasConflict = await page.evaluate(async () => {
            ...
            for (let r = 0; r < size; r++) {
                for (let c = 0; c < size; c++) {
                    ...
                    // Find a filled king-diagonal neighbor
                    for (const [dr, dc] of offsets) {
                        const nr = r + dr, nc = c + dc;
    ...
    test('notes are cleared on king-diagonal cells when number is placed', async ({ page }) => {
        await startNewGame(page, 'easy', { variant: 'anti-king' });
    ...
    test('anti-king variant persists after save and restore', async ({ page }) => {
        await startNewGame(page, 'easy', { variant: 'anti-king' });
    ...
    test('completed anti-king game appears in history with badge', async ({ page }) => {
        await startNewGame(page, 'easy', { variant: 'anti-king' });
        ...
        await expect(page.locator('.badge-anti-king').first()).toBeVisible();
    });

    test('history entry stores anti-king variant', async ({ page }) => {
        await startNewGame(page, 'easy', { variant: 'anti-king' });
        await solveEntirePuzzle(page);
        await page.waitForSelector('#screen-complete.active', { timeout: 10000 });
        ...
        expect(historyVariant).toBe('anti-king');
    });

    // --- Replay ---

    test('replay preserves anti-king variant', async ({ page }) => {
        await startNewGame(page, 'easy', { variant: 'anti-king' });
        await solveEntirePuzzle(page);
        await page.waitForSelector('#screen-complete.active', { timeout: 10000 });
        await page.click('[data-navigate="main"]');
    ...
    test('replay anti-king game can be completed', async ({ page }) => {
        await startNewGame(page, 'easy', { variant: 'anti-king' });
        await solveEntirePuzzle(page);
        await page.waitForSelector('#screen-complete.active', { timeout: 10000 });
        await page.click('[data-navigate="main"]');
    ...
    test('standard game is not affected by anti-king variant', async ({ page }) => {
